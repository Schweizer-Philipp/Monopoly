...
public class GemeinschaftsFeldPanel extends JPanel {
    ...
    private void initVars() {
        ...
        gFelder = DateiManager.dateiLesen("Gemeinschaftskarten.txt");
        lblbild = new JLabel();
        lblname = new JLabel();
    ...
    public void setUpGemeinschaftsfeld(String nummer, boolean status, int spieler, GameManager gameManager) {
        ...
        for (String s : gFelder) {
            ...
            if (test[0].equals(nummer)) {
                setUpJlabel(lblname, "Gemeinschaftsfeld");
                setUpJlabel(lblbild, "Gemfeld.png");
                setUpJlabel(lblUeberschrift, test[1]);
                setUpJlabel(txtftxt, test[2]);
                if (status) {

                    JOptionPane.showMessageDialog(null, test[1], "Gemeinschaftsfeld", JOptionPane.INFORMATION_MESSAGE);
                    if (test[3].equals("karte")) {
                        ...
                        for (StreetBuyAble street : SpielerManager.getSpielerarray()[spieler - 1].getStreet()) {
                            if (street.getHaeuserAnzahl() == 5) {
                                ...
                                betrag = betrag + street.getHaeuserAnzahl() * 40;
                            }
                        }
                        ...
                        gameManager.setSpielerPanel();
                        gameManager.setSpielerStrassenPanel();
                        JOptionPane.showMessageDialog(null, "Sie haben " + test[3] + "EUR gemacht", "zahlen", JOptionPane.INFORMATION_MESSAGE);
                        if (SpielerManager.getSpielerarray()[spieler - 1].getGeld() <= 0) {
                            gameManager.spielerPleite(spieler);
                        }
                    }
